...
import { INTEGRATION, EDIT, prepareEditProfile } from '../../../gql/profile';
...
class GEDSAudit extends Component {
  ...
  handleChange(e) {
    ...
    if (name === 'auditAddress') {
      if (id === 'auditAddress-1') {
        this.setState({
          streetAddress: this.state.gStreetAddress,
          city: this.state.gCity,
          province: this.state.gProvince,
          country: this.state.gCountry,
          postalCode: this.state.gPostalCode,
        });
      } else if (id === 'auditAddress-2') {
        this.setState({
